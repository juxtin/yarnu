...
class DBStorage {
public:
    typedef std::function<void(std::vector<winrt::Microsoft::ReactNative::JSValue> const&)> Callback;

    class DBTask {
    public:
        enum class Type { multiGet, multiSet, multiRemove, clear, getAllKeys };
        DBTask(Type type, std::vector<winrt::Microsoft::ReactNative::JSValue>&& args,
            Callback&& callback)
    ...
    void AddTask(DBTask::Type type, std::vector<winrt::Microsoft::ReactNative::JSValue>&& args, Callback&& jsCallback);
    void AddTask(DBTask::Type type, Callback&& jsCallback) {
        AddTask(type, std::move(std::vector<winrt::Microsoft::ReactNative::JSValue>()), std::move(jsCallback));
    }
